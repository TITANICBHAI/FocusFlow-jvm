...
> Version 1.1.2 · MSIX Identity confirmed · All Partner Center fields verified in CI
...
| Version | `1.1.2.0` (4th digit must be 0 for Store) |
...
The manifest is auto-generated in CI with all 3 Partner Center identity fields and is verified before `makeappx` runs. The output file is `FocusFlow-1.1.2-x64.msix`.
...
1. Upload `FocusFlow-1.1.2-x64.msix` under **Packages**.
...
> The Microsoft Store requires each submission's version to be **greater than** the previous. Current version is `1.1.2.0`. Next submission must be `1.1.2.0` or higher.
