    ...
    fun getRunningApps(): List<ScannedApp> {
        val running = try {
            ProcessHandle.allProcesses()
                ...
                .map { p ->
                    val cmd = p.info().command().get()
                    val exe = java.io.File(cmd).name.lowercase()
                    ScannedApp(exe, friendlyName(exe), isRunning = true)
                }
                .filter { it.processName.isNotBlank() && it.processName !in systemIgnore }
                .distinctBy { it.processName }
                .toList()
        } catch (_: Exception) { emptyList() }

        val runningNames = running.map { it.processName }.toSet()
        val notRunning = curated.filter { it.processName !in runningNames }

        return (running + notRunning)
            .distinctBy { it.processName.lowercase() }
            .sortedWith(compareByDescending<ScannedApp> { it.isRunning }.thenBy { it.displayName })
    }

    private fun friendlyName(exe: String): String {
        return curated.find { it.processName.equals(exe, ignoreCase = true) }?.displayName
