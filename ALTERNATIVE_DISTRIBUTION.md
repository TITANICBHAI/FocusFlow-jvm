...
> Version 1.1.2 · Windows x64 · TBTechs
...
1. Build the installer locally:
   ```bash
   gradle packageExe
   # Output: build/compose/binaries/main/exe/FocusFlow-1.1.2.exe
   ```
2. Go to `https://github.com/TITANICBHAI/FocusFlow-jvm/releases/new`
3. Tag: `v1.1.2`
4. Release title: `FocusFlow v1.1.2 — Deep Focus & App Blocker`
5. Upload: `FocusFlow-1.1.2.exe` and `FocusFlow-1.1.2.msi`
...
### Install
Download FocusFlow-1.1.2.exe and run it. No Java installation required — runtime is bundled.
```
...
2. Create the folder path:
   ```
   manifests/t/TBTechs/FocusFlow/1.1.2/
   ```
...
Installers:
  - Architecture: x64
    InstallerUrl: https://github.com/TITANICBHAI/FocusFlow-jvm/releases/download/v1.1.2/FocusFlow-1.1.2.exe
    InstallerSha256: <SHA256_OF_EXE>   # run: certutil -hashfile FocusFlow-1.1.2.exe SHA256
    InstallerSwitches:
      Silent: /S
...
4. Open a PR to `microsoft/winget-pkgs` — title: `New package: TBTechs.FocusFlow version 1.1.2`
5. The winget bot validates automatically. Usually approved within 1–3 days.

> **SHA256 tip:** Generate with `certutil -hashfile FocusFlow-1.1.2.exe SHA256` on Windows or `sha256sum` on Linux.
...
$installerType = 'exe'
$url64 = 'https://github.com/TITANICBHAI/FocusFlow-jvm/releases/download/v1.1.2/FocusFlow-1.1.2.exe'
...
4. Pack and push:
   ...
   choco push focusflow.1.1.2.nupkg --source https://push.chocolatey.org --api-key <YOUR_API_KEY>
   ```
...
```json
{
  ...
  "architecture": {
    "64bit": {
      "url": "https://github.com/TITANICBHAI/FocusFlow-jvm/releases/download/v1.1.2/FocusFlow-1.1.2.exe",
      "hash": "<SHA256_OF_EXE>"
    }
  },
  "installer": {
    "script": "Start-Process -FilePath \"$dir\\FocusFlow-1.1.2.exe\" -ArgumentList '/S' -Wait"
  },
  "checkver": {
...
```bash
# Windows (PowerShell)
Get-FileHash FocusFlow-1.1.2.exe -Algorithm SHA256

# Windows (CMD)
certutil -hashfile FocusFlow-1.1.2.exe SHA256

# Linux / macOS
sha256sum FocusFlow-1.1.2.exe
```
